...
export default {
  ...
  script: async (id) => {
    try {
      // Se carga el proyecto real desde la base de datos
      ...
      // Convertir la fecha para establecer el valor en el input date
      const fechaCorta = proyecto.created_at.split("T")[0];

      // Inyectamos los datos del proyecto en el formulario
      document
      ...
      // Actualización de la imagen a partir de la URL ingresada
      const inputUrl = document.querySelector("#urlImagen");
      inputUrl.addEventListener("input", () => {
        document
          .querySelector("#imagenJuego")
          .setAttribute("src", inputUrl.value);
      });

      // Validación BOOTSTRAP y envío del formulario
      ...
      formulario.addEventListener("submit", async (event) => {
        ...
        if (!formulario.checkValidity()) {
          ...
          try {
            ...
            // Llama al método update para actualizar el proyecto existente
